...
var todoeng = {
	createItem: function(req, res) {
		...
		var query = "INSERT INTO items (descripcion, fecha_creacion) values ($1, now())";

		db.none(query, [description])
			.then(function(data) {
				// volvemos a retomar toooodas los inserts
				var items = [];

				db.many("SELECT item_id, descripcion, hecho from items")
					.then(function(data){
	...
	changeState: function(req, res) {
		...
		var query = "UPDATE items SET (fecha_terminado, hecho) = (now(), (select not(hecho) from items where item_id = $1)) where item_id = $1";
	...
	deleteItem: function(req, res) {
		...
		var query = "DELETE FROM items WHERE item_id = $1";
